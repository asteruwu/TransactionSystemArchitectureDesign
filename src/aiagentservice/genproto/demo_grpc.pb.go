...
func (c *productCatalogServiceClient) ChargeProduct(ctx context.Context, in *ChargeProductRequest, opts ...grpc.CallOption) (*ChargeProductResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ChargeProductResponse)
	err := c.cc.Invoke(ctx, ProductCatalogService_ChargeProduct_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _ProductCatalogService_ChargeProduct_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ChargeProductRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(ProductCatalogServiceServer).ChargeProduct(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: ProductCatalogService_ChargeProduct_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(ProductCatalogServiceServer).ChargeProduct(ctx, req.(*ChargeProductRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "hipstershop.UserService",
...
const (
	AIAgentService_Chat_FullMethodName = "/hipstershop.AIAgentService/Chat"
)
...
func (c *aIAgentServiceClient) Chat(ctx context.Context, in *ChatRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[ChatResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type AIAgentService_ChatClient = grpc.ServerStreamingClient[ChatResponse]
...
func (UnimplementedAIAgentServiceServer) Chat(*ChatRequest, grpc.ServerStreamingServer[ChatResponse]) error {
...
func _AIAgentService_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(AIAgentServiceServer).Chat(m, &grpc.GenericServerStream[ChatRequest, ChatResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AIAgentService_ChatServer = grpc.ServerStreamingServer[ChatResponse]
...
var AIAgentService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "hipstershop.AIAgentService",
